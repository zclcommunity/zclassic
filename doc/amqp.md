...
The Zclassic daemon can be configured to act as a trusted "border
router", implementing the Zclassic P2P protocol and relay, making
...
The AMQP feature in Zclassic requires [Qpid Proton](https://qpid.apache.org/proton/)
...
applications that you're running on the same computer as Zclassicd to
...
Launch zclassicd like this:

    $ zclassicd -amqppubhashtx=amqp://127.0.0.1:5672
...
From the perspective of zclassicd, the local end of an AMQP link is write-only.

No information is broadcast that wasn't already received from the public
P2P network.

No authentication or authorization is done on peers that zclassicd connects
to; it is assumed that the AMQP link is exposed only to trusted entities,
using other means such as firewalling.

TLS support may be added once OpenSSL has been removed from the Zclassic
...
At present, zclassicd does not try to resend a notification if there was
...
Currently, zclassicd appends an up-counting sequence number to each notification
