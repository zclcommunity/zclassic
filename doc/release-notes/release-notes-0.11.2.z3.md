...
Nathan Wilcox (4):
      ...
      Add a zclassic-gtest binary to our build with a single tautological test.
      Add coverage support scoped to only the zclassic-gtest run; invoke with make zclassic-cov; make cov is a superset.
...
Sean Bowe (49):
      ...
      Move incremental merkle tree tests to zclassic-gtest.
      ...
      Small nit fixes
      Run `zclassic-gtest` in `make check` and fix performance tests.
      ...
      Introduce new `libzclassic` Zclassic protocol API and crypto constructions surrounding the zkSNARK circuit.
      ...
      Rename ZCLASSIC_ constants to ZC_.
...
Taylor Hornby (7):
      ...
      Fix RPC tests
      Rename bitcoin.conf and bitcoind.pid to zclassic.conf and zclassicd.pid in qa/ and src/
