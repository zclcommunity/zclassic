...
[Pull request](https://github.com/zclassic/zclassic/pull/3559)
...
[Pull request](https://github.com/zclassic/zclassic/pull/3492), [ZIP 32](https://github.com/zclassic/zips/blob/master/zip-0032.mediawiki)

Fix Signing Raw Transactions with Unsynced Offline Nodes
--------------------------------------------------------
With v2.0.0, in `signrawtransaction` the consensus branch ID (which is used to construct the transaction) was estimated using the chain height. With v2.0.1 this has been improved by also considering the `APPROX_RELEASE_HEIGHT` of the release, and a new parameter to allow the caller to manually override the consensus branch ID that zclassicd will use.

[Pull request](https://github.com/zclassic/zclassic/pull/3520)
...
Jack Grigg (83):
      ...
      Clean up libzclassic CPPFLAGS, CXXFLAGS, and LDFLAGS
      ...
      Metrics UI: Tell Windows users how to stop zclassicd
...
Jonas Schnelli (3):
      [Wallet] extend CKeyMetadata with HD keypath     Zclassic: modified for zip32
      [Wallet] print hd masterkeyid in getwalletinfo     Zclassic: modified for zip32
      [Wallet] ensure CKeyMetadata.hdMasterKeyID will be cleared during SetNull()     Zclassic: modified for zip32
