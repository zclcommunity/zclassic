...
If you see the following error when updating to a new version of zclassicd:
...
`wget -qO - https://apt.z.cash/zclassic.asc | sudo apt-key add -`

Then update the package lists:

`sudo apt-get update`

[Issue](https://github.com/zclassic/zclassic/issues/3612)
...
Jonathan "Duke" Leto (2):
      ...
      Fix another instance of incorrectly saying amount is double precision, and s/ZC/ZCL/
...
Simon Liu (14):
      ...
      For ZCL-013. Mitigate potential tx expiry height related DoS vector.
      For ZCL-013. Don't propagate txs which are expiring soon in p2p messages.
      For ZCL-013. RPC createrawtransaction returns error if tx expiring soon.
      For ZCL-013. Update qa tests broken by expiring soon threshold.
      For ZCL-013. RPC sendrawtransaction returns error if tx expiring soon.
