...
Jack Grigg (22):
      ...
      Instruct users to run zclassic-fetch-params if network params aren't available
      ...
      Throw an error if zclassic.conf is missing
      Show a friendly message explaining why zclassicd needs a zclassic.conf
      Fix gtest ordering broken by #1949
      Debian package lint
      Generate Debian control file to fix shlibs lint
      Create empty zclassic.conf during performance measurements
      Create empty zclassic.conf during coverage checks
...
Simon Liu (13):
      ...
      Remove UTF-8 BOM efbbbf from zclassic.conf to avoid problems with command line tools
      Closes #1097 so zclassic-cli now displays license info like zclassicd.
