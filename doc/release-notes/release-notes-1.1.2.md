...
In release 1.0.9 we implemented automatic deprecation of `zclassicd` software
versions made by the Zclassic Company. The configuration option
...
This release removes the `-disabledeprecation` option, so that `zclassicd` software
versions made by the Zclassic Company will always shut down in accordance with the
...
supported by the Zclassic Company.
