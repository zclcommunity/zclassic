...
{
    // Copied from GetDefaultDataDir and adapter for zclassic params.

    namespace fs = boost::filesystem;
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ZclassicParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ZclassicParams
    // Mac: ~/Library/Application Support/ZclassicParams
    // Unix: ~/.zcash-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ZclassicParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
