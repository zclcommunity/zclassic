...
#include "streams.h"
#include "version.h"
#include "zclassic/prf.h"

#include <librustzcash.h>
#include <sodium.h>

const unsigned char ZCLASSIC_HD_SEED_FP_PERSONAL[crypto_generichash_blake2b_PERSONALBYTES] =
    {'Z', 'c', 'a', 's', 'h', '_', 'H', 'D', '_', 'S', 'e', 'e', 'd', '_', 'F', 'P'};

const unsigned char ZCLASSIC_TADDR_OVK_PERSONAL[crypto_generichash_blake2b_PERSONALBYTES] =
...
{
    CBLAKE2bWriter h(SER_GETHASH, 0, ZCLASSIC_HD_SEED_FP_PERSONAL);
    h << seed;
    return h.GetHash();
}
...
boost::optional<std::pair<diversifier_index_t, libzclassic::SaplingPaymentAddress>>
    SaplingExtendedFullViewingKey::Address(diversifier_index_t j) const
{
    ...
    CSerializeData addr_bytes(libzclassic::SerializedSaplingPaymentAddressSize);
...
libzclassic::SaplingPaymentAddress SaplingExtendedFullViewingKey::DefaultAddress() const
{
...
libzclassic::SaplingPaymentAddress SaplingExtendedSpendingKey::DefaultAddress() const
{
...
bool IsValidSpendingKey(const libzclassic::SpendingKey& zkey) {
    return zkey.which() != 0;
}
