...
const unsigned char ZCLASSIC_SAPLING_FVFP_PERSONALIZATION[crypto_generichash_blake2b_PERSONALBYTES] =
...
uint256 SaplingFullViewingKey::GetFingerprint() const {
    CBLAKE2bWriter ss(SER_GETHASH, 0, ZCLASSIC_SAPLING_FVFP_PERSONALIZATION);
    ss << *this;
    return ss.GetHash();
}
...
bool IsValidPaymentAddress(const libzclassic::PaymentAddress& zaddr) {
    return zaddr.which() != 0;
}

bool IsValidViewingKey(const libzclassic::ViewingKey& vk) {
    return vk.which() != 0;
}
