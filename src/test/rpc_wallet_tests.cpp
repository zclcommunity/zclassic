...
BOOST_AUTO_TEST_CASE(rpc_wallet_z_exportwallet)
{
    ...
    libzclassic::SproutPaymentAddress addr = pwalletMain->GenerateNewSproutZKey();
    pwalletMain->GetSproutPaymentAddresses(addrs);
...
BOOST_AUTO_TEST_CASE(rpc_wallet_z_importwallet)
{
    ...
    auto testSpendingKey = libzclassic::SproutSpendingKey::random();
    ...
    BOOST_ASSERT(boost::get<libzclassic::SproutPaymentAddress>(&address) != nullptr);
    auto addr = boost::get<libzclassic::SproutPaymentAddress>(address);
...
BOOST_AUTO_TEST_CASE(rpc_wallet_z_importexport)
{
    ...
    pwalletMain->GetSproutPaymentAddresses(addrs);
    BOOST_CHECK(addrs.size()==0);
    std::set<libzclassic::SaplingPaymentAddress> saplingAddrs;
    ...
    BOOST_CHECK(IsValidPaymentAddress(address));
    BOOST_ASSERT(boost::get<libzclassic::SproutPaymentAddress>(&address) != nullptr);
    auto newAddr = boost::get<libzclassic::SproutPaymentAddress>(address);
...
BOOST_AUTO_TEST_CASE(rpc_wallet_encrypted_wallet_sapzkeys)
{
    ...
    // wallet should currently be empty
    std::set<libzclassic::SaplingPaymentAddress> addrs;
