...
BOOST_AUTO_TEST_CASE(test_basic_joinsplit_verification)
{
    ...
    //
    // Also, it's generally libzclassic's job to ensure the
    // integrity of the scheme through its own tests.

    // construct a merkle tree
    SproutMerkleTree merkleTree;

    auto k = libzclassic::SproutSpendingKey::random();
    auto addr = k.address();
    ...
    {
        JSDescription jsdesc(false, *pzclassicParams, joinSplitPubKey, rt, inputs, outputs, 0, 0);
        BOOST_CHECK(jsdesc.Verify(*pzclassicParams, verifier, joinSplitPubKey));
        ...
        BOOST_CHECK(jsdesc_deserialized.Verify(*pzclassicParams, verifier, joinSplitPubKey));
    }

    {
        // Ensure that the balance equation is working.
        BOOST_CHECK_THROW(JSDescription(false, *pzclassicParams, joinSplitPubKey, rt, inputs, outputs, 10, 0), std::invalid_argument);
        BOOST_CHECK_THROW(JSDescription(false, *pzclassicParams, joinSplitPubKey, rt, inputs, outputs, 0, 10), std::invalid_argument);
    }

    {
        // Ensure that it won't verify if the root is changed.
        auto test = JSDescription(false, *pzclassicParams, joinSplitPubKey, rt, inputs, outputs, 0, 0);
        test.anchor = GetRandHash();
        BOOST_CHECK(!test.Verify(*pzclassicParams, verifier, joinSplitPubKey));
    }
}
