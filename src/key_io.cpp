...
public:
    ...
    std::string operator()(const libzclassic::SproutPaymentAddress& zaddr) const
    {
    ...
    std::string operator()(const libzclassic::SaplingPaymentAddress& zaddr) const
    {
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
        ss << zaddr;
    ...
    std::string operator()(const libzclassic::InvalidEncoding& no) const { return {}; }
};
...
public:
    ...
    std::string operator()(const libzclassic::SproutViewingKey& vk) const
    {
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
        ss << vk;
...
public:
    ...
    std::string operator()(const libzclassic::SproutSpendingKey& zkey) const
    {
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
        ss << zkey;
    ...
    std::string operator()(const libzclassic::SaplingExtendedSpendingKey& zkey) const
    {
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
        ss << zkey;
        // ConvertBits requires unsigned char, but CDataStream uses char
...
libzclassic::PaymentAddress DecodePaymentAddress(const std::string& str)
{
    ...
    if (DecodeBase58Check(str, data)) {
        ...
        if ((data.size() == libzclassic::SerializedSproutPaymentAddressSize + zaddr_prefix.size()) &&
            ...
            libzclassic::SproutPaymentAddress ret;
            ss >> ret;
            return ret;
        }
    }
    ...
    if (bech.first == Params().Bech32HRP(CChainParams::SAPLING_PAYMENT_ADDRESS) &&
        ...
        if (ConvertBits<5, 8, false>([&](unsigned char c) { data.push_back(c); }, bech.second.begin(), bech.second.end())) {
            ...
            libzclassic::SaplingPaymentAddress ret;
            ss >> ret;
            return ret;
        }
    }
    return libzclassic::InvalidEncoding();
}
...
libzclassic::SpendingKey DecodeSpendingKey(const std::string& str)
{
    ...
    if (bech.first == Params().Bech32HRP(CChainParams::SAPLING_EXTENDED_SPEND_KEY) &&
        ...
        if (ConvertBits<5, 8, false>([&](unsigned char c) { data.push_back(c); }, bech.second.begin(), bech.second.end())) {
            CDataStream ss(data, SER_NETWORK, PROTOCOL_VERSION);
            libzclassic::SaplingExtendedSpendingKey ret;
            ss >> ret;
            memory_cleanse(data.data(), data.size());
            return ret;
        }
