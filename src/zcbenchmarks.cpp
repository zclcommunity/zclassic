...
double benchmark_verify_joinsplit(const JSDescription &joinsplit)
{
    ...
    uint256 joinSplitPubKey;
    auto verifier = libzclassic::ProofVerifier::Strict();
    joinsplit.Verify(*pzclassicParams, verifier, joinSplitPubKey);
    return timer_stop(tv_start);
}
...
double benchmark_try_decrypt_sprout_notes(size_t nKeys)
{
    ...
    auto sk = libzclassic::SproutSpendingKey::random();
    auto tx = GetValidSproutReceive(*pzclassicParams, sk, 10, true);
...
CWalletTx CreateSproutTxWithNoteData(const libzclassic::SproutSpendingKey& sk) {
    auto wtx = GetValidSproutReceive(*pzclassicParams, sk, 10, true);
    auto note = GetSproutNote(*pzclassicParams, sk, wtx, 0, 1);
...
double benchmark_increment_sprout_note_witnesses(size_t nTxs)
{
    ...
    auto sproutSpendingKey = libzclassic::SproutSpendingKey::random();
...
double benchmark_create_sapling_output()
{
    ...
    libzclassic::SaplingNotePlaintext notePlaintext(note, memo);
