...
CMutableTransaction GetValidSproutReceiveTransaction(ZCJoinSplit& params,
    ...
    std::array<libzclassic::JSInput, 2> inputs = {
        libzclassic::JSInput(), // dummy input
        libzclassic::JSInput() // dummy input
    };

    std::array<libzclassic::JSOutput, 2> outputs = {
        libzclassic::JSOutput(sk.address(), value),
        libzclassic::JSOutput(sk.address(), value)
    };
...
CWalletTx GetValidSproutSpend(ZCJoinSplit& params,
    ...
    libzclassic::JSOutput dummyout;
    libzclassic::JSInput dummyin;

    {
        if (note.value() > value) {
            libzclassic::SproutSpendingKey dummykey = libzclassic::SproutSpendingKey::random();
            libzclassic::SproutPaymentAddress dummyaddr = dummykey.address();
            dummyout = libzclassic::JSOutput(dummyaddr, note.value() - value);
        } else if (note.value() < value) {
            libzclassic::SproutSpendingKey dummykey = libzclassic::SproutSpendingKey::random();
            libzclassic::SproutPaymentAddress dummyaddr = dummykey.address();
            libzclassic::SproutNote dummynote(dummyaddr.a_pk, (value - note.value()), uint256(), uint256());
            tree.append(dummynote.cm());
            dummyin = libzclassic::JSInput(tree.witness(), dummynote, dummykey);
        }
    }
    ...
    std::array<libzclassic::JSInput, 2> inputs = {
        libzclassic::JSInput(tree.witness(), note, sk),
        dummyin
    };

    std::array<libzclassic::JSOutput, 2> outputs = {
        dummyout, // dummy output
        libzclassic::JSOutput() // dummy output
    };
...
libzclassic::SaplingExtendedSpendingKey GetTestMasterSaplingSpendingKey() {
    ...
    HDSeed seed(rawSeed);
    return libzclassic::SaplingExtendedSpendingKey::Master(seed);
}
...
TestSaplingNote GetTestSaplingNote(const libzclassic::SaplingPaymentAddress& pa, CAmount value) {
    ...
    libzclassic::SaplingNote note(pa, value);
