...
TEST(Keys, EncodeAndDecodeSapling)
{
    ...
    for (uint32_t i = 0; i < 1000; i++) {
        auto sk = m.Derive(i);
        {
            ...
            ASSERT_TRUE(boost::get<libzclassic::SaplingExtendedSpendingKey>(&spendingkey2) != nullptr);
            auto sk2 = boost::get<libzclassic::SaplingExtendedSpendingKey>(spendingkey2);
            EXPECT_EQ(sk, sk2);
        }
        {
            ...
            ASSERT_TRUE(boost::get<libzclassic::SaplingPaymentAddress>(&paymentaddr2) != nullptr);
            auto addr2 = boost::get<libzclassic::SaplingPaymentAddress>(paymentaddr2);
