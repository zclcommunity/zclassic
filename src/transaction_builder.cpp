    ...
    std::array<unsigned char, ZC_MEMO_SIZE> memo)
{
    ...
    auto note = libzclassic::SaplingNote(to, value);
    ...
    std::array<unsigned char, ZC_MEMO_SIZE> memo)
{
    ...
    libzclassic::JSOutput jsOutput(to, value);
    jsOutput.memo = memo;
...
void TransactionBuilder::SendChangeTo(libzclassic::SaplingPaymentAddress changeAddr, uint256 ovk)
{
...
void TransactionBuilder::SendChangeTo(libzclassic::SproutPaymentAddress changeAddr)
{
...
TransactionBuilderResult TransactionBuilder::Build()
{
    ...
    if (change > 0) {
        ...
        if (saplingChangeAddr) {
            ...
            libzclassic::SaplingPaymentAddress changeAddr(note.d, note.pk_d);
    ...
    for (auto output : outputs) {
        ...
        libzclassic::SaplingNotePlaintext notePlaintext(output.note, output.memo);
        ...
        libzclassic::SaplingOutgoingPlaintext outPlaintext(output.note.pk_d, encryptor.get_esk());
...
void TransactionBuilder::CreateJSDescriptions()
{
    ...
    std::deque<libzclassic::JSInput> jsInputsDeque;
    for (auto jsInput : jsInputs) {
        jsInputsDeque.push_back(jsInput);
    }
    std::deque<libzclassic::JSOutput> jsOutputsDeque;
    for (auto jsOutput : jsOutputs) {
    ...
    while (!vpubNewProcessed) {
        ...
        if (jsChange > 0) {
            ...
            try {
                auto plaintext = libzclassic::SproutNotePlaintext::decrypt(
                    decryptor,
                ...
                vjsin[0] = libzclassic::JSInput(changeWitness.get(), note, changeKey);
        ...
        libzclassic::JSOutput recipient;
        ...
        if (jsOutputsDeque.empty() && jsInputsDeque.empty()) {
            ...
            if (jsInputValue > outAmount) {
                ...
                libzclassic::JSOutput recipientDue(recipient.addr, due);
        ...
        if (jsChange > 0) {
            vjsout[1] = libzclassic::JSOutput(changeAddress, jsChange);
