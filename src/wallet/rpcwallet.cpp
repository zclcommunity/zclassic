...
UniValue z_listunspent(const UniValue& params, bool fHelp)
{
    ...
    std::set<libzclassic::PaymentAddress> zaddrs = {};
    ...
    if (params.size() > 3) {
        ...
        std::set<libzclassic::SproutPaymentAddress> sproutzaddrs = {};
        pwalletMain->GetSproutPaymentAddresses(sproutzaddrs);
        
        // Sapling support
        std::set<libzclassic::SaplingPaymentAddress> saplingzaddrs = {};
    ...
    if (zaddrs.size() > 0) {
        ...
        for (auto & entry : sproutEntries) {
            ...
            bool hasSproutSpendingKey = pwalletMain->HaveSproutSpendingKey(boost::get<libzclassic::SproutPaymentAddress>(entry.address));
        ...
        for (auto & entry : saplingEntries) {
            ...
            pwalletMain->GetSaplingIncomingViewingKey(boost::get<libzclassic::SaplingPaymentAddress>(entry.address), ivk);
...
UniValue zc_raw_receive(const UniValue& params, bool fHelp)
{
    ...
    if (boost::get<libzclassic::SproutSpendingKey>(&spendingkey) == nullptr) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Only works with Sprout spending keys");
    }
    SproutSpendingKey k = boost::get<libzclassic::SproutSpendingKey>(spendingkey);
...
UniValue zc_raw_joinsplit(const UniValue& params, bool fHelp)
{
    ...
    for (const string& name_ : outputs.getKeys()) {
        ...
        if (boost::get<libzclassic::SproutPaymentAddress>(&addrTo) == nullptr) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Only works with Sprout payment addresses");
        }
        CAmount nAmount = AmountFromValue(outputs[name_]);

        vjsout.push_back(JSOutput(boost::get<libzclassic::SproutPaymentAddress>(addrTo), nAmount));
    }
    ...
    {
        auto verifier = libzclassic::ProofVerifier::Strict();
        assert(jsdesc.Verify(*pzclassicParams, verifier, joinSplitPubKey));
    }
    ...
    std::string encryptedNote2;
    {
        ...
        ss2 << jsdesc.h_sig(*pzclassicParams, joinSplitPubKey);
...
UniValue z_listaddresses(const UniValue& params, bool fHelp)
{
    ...
    UniValue ret(UniValue::VARR);
    {
        std::set<libzclassic::SproutPaymentAddress> addresses;
    ...
    {
        std::set<libzclassic::SaplingPaymentAddress> addresses;
...
UniValue z_listreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    if (boost::get<libzclassic::SproutPaymentAddress>(&zaddr) != nullptr) {
    ...
    } else if (boost::get<libzclassic::SaplingPaymentAddress>(&zaddr) != nullptr) {
...
UniValue z_sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (!fromTaddr) {
        ...
        fromSapling = boost::get<libzclassic::SaplingPaymentAddress>(&res) != nullptr;
    }
    ...
    for (const UniValue& o : outputs.getValues()) {
        ...
        if (!IsValidDestination(taddr)) {
            ...
            if (IsValidPaymentAddress(res)) {
                ...
                bool toSapling = boost::get<libzclassic::SaplingPaymentAddress>(&res) != nullptr;
...
UniValue z_mergetoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (!IsValidDestination(taddr)) {
        ...
        if (IsValidPaymentAddress(decodeAddr)) {
            if (boost::get<libzclassic::SaplingPaymentAddress>(&decodeAddr) != nullptr) {
