...
bool AsyncRPCOperation_mergetoaddress::main_impl()
{
    ...
    if (isUsingBuilder_) {
        ...
        if (isToTaddr_) {
            ...
            auto saplingPaymentAddress = boost::get<libzclassic::SaplingPaymentAddress>(&toPaymentAddress_);
    ...
    if (sproutNoteInputs_.empty() && isToZaddr_) {
        ...
        JSOutput jso = JSOutput(boost::get<libzclassic::SproutPaymentAddress>(toPaymentAddress_), sendAmount);
        if (hexMemo.size() > 0) {
    ...
    while (!vpubNewProcessed) {
        ...
        if (jsChange > 0) {
            ...
            if (isToZaddr_ && vpubNewProcessed) {
                outputType = "target";
                jso.addr = boost::get<libzclassic::SproutPaymentAddress>(toPaymentAddress_);
                if (!hexMemo.empty()) {
                    jso.memo = get_memo_from_hex_string(hexMemo);
                }
            }
    ...
    uint256 anchor)
{
    ...
    std::array<libzclassic::JSInput, ZC_NUM_JS_INPUTS> inputs{info.vjsin[0], info.vjsin[1]};
    std::array<libzclassic::JSOutput, ZC_NUM_JS_OUTPUTS> outputs{info.vjsout[0], info.vjsout[1]};
