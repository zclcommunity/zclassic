...
bool AsyncRPCOperation_saplingmigration::main_impl() {
    ...
    // If the remaining amount to be migrated is less than 0.01 ZCL, end the migration.
    ...
    libzclassic::SaplingPaymentAddress migrationDestAddress = getMigrationDestAddress(seed);
    ...
    do {
        ...
        auto builder = TransactionBuilder(consensusParams, targetHeight_, MIGRATION_EXPIRY_DELTA, pwalletMain, pzclassicParams,
                                          &coinsView, &cs_main);
        ...
        for (const SproutNoteEntry& sproutEntry : fromNotes) {
            ...
            libzclassic::SproutSpendingKey sproutSk;
            ...
            // TODO: the above functionality (in comment) is not implemented in zclassicd
            uint256 inputAnchor;
            std::vector<boost::optional<SproutWitness>> vInputWitnesses;
            pwalletMain->GetSproutNoteWitnesses(vOutPoints, vInputWitnesses, inputAnchor);
            builder.AddSproutInput(sproutSk, sproutEntry.note, vInputWitnesses[0].get());
        }
        // The amount chosen *includes* the 0.0001 ZCL fee for this transaction, i.e.
        // the value of the Sapling output will be 0.0001 ZCL less.
        builder.SetFee(FEE);
...
libzclassic::SaplingPaymentAddress AsyncRPCOperation_saplingmigration::getMigrationDestAddress(const HDSeed& seed) {
    if (mapArgs.count("-migrationdestaddress")) {
        ...
        auto saplingAddress = boost::get<libzclassic::SaplingPaymentAddress>(&address);
    ...
    libzclassic::SaplingExtendedSpendingKey xsk = m_32h_cth.Derive(0 | ZIP32_HARDENED_KEY_LIMIT);

    libzclassic::SaplingPaymentAddress toAddress = xsk.DefaultAddress();
