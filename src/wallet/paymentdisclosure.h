...
#ifndef ZCLASSIC_PAYMENTDISCLOSURE_H
#define ZCLASSIC_PAYMENTDISCLOSURE_H
...
struct PaymentDisclosureInfo {
    ...
    uint256 esk;              // zclassic/NoteEncryption.cpp
    uint256 joinSplitPrivKey; // primitives/transaction.h
    // ed25519 - not tied to implementation e.g. libsodium, see ed25519 rfc

    libzclassic::SproutPaymentAddress zaddr;

    PaymentDisclosureInfo() : version(PAYMENT_DISCLOSURE_VERSION_EXPERIMENTAL) {
    }

    PaymentDisclosureInfo(uint8_t v, uint256 esk, uint256 key, libzclassic::SproutPaymentAddress zaddr) : version(v), esk(esk), joinSplitPrivKey(key), zaddr(zaddr) { }
...
struct PaymentDisclosurePayload {
    ...
    libzclassic::SproutPaymentAddress zaddr; // zclassic/Address.hpp
...
#endif // ZCLASSIC_PAYMENTDISCLOSURE_H
