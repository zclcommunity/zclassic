...
bool AsyncRPCOperation_sendmany::main_impl() {
    ...
    if (isUsingBuilder_) {
        ...
        uint256 ovk;
        if (isfromzaddr_) {
            auto sk = boost::get<libzclassic::SaplingExtendedSpendingKey>(spendingkey_);
            expsk = sk.expsk;
        ...
        for (auto r : z_outputs_) {
            ...
            assert(boost::get<libzclassic::SaplingPaymentAddress>(&addr) != nullptr);
            auto to = boost::get<libzclassic::SaplingPaymentAddress>(addr);
    ...
    if (isfromtaddr_) {
        ...
        while (zOutputsDeque.size() > 0) {
            ...
            while (n++<ZC_NUM_JS_OUTPUTS && zOutputsDeque.size() > 0) {
                ...
                JSOutput jso = JSOutput(boost::get<libzclassic::SproutPaymentAddress>(pa), value);
                if (hexMemo.size() > 0) {
    ...
    while (!vpubNewProcessed) {
        ...
        if (jsChange > 0) {
            ...
            ZCNoteDecryption decryptor(boost::get<libzclassic::SproutSpendingKey>(spendingkey_).receiving_key());
            auto hSig = prevJoinSplit.h_sig(*pzclassicParams, tx_.joinSplitPubKey);
            try {
                ...
                SproutNote note = plaintext.note(boost::get<libzclassic::SproutPaymentAddress>(frompaymentaddress_));
        ...
        if (jsChange>0) {
            info.vjsout.push_back(JSOutput(boost::get<libzclassic::SproutPaymentAddress>(frompaymentaddress_), jsChange));
        ...
        uint256 anchor)
{
    ...
    for (size_t i = 0; i < witnesses.size(); i++) {
        ...
        info.vjsin.push_back(JSInput(*witnesses[i], info.notes[i], boost::get<libzclassic::SproutSpendingKey>(spendingkey_)));
    }
            ...
            &esk); // parameter expects pointer to esk, so pass in address
    {
        ...
        if (!(jsdesc.Verify(*pzclassicParams, verifier, joinSplitPubKey_))) {
    ...
    std::string encryptedNote2;
    {
        ...
        ss2 << jsdesc.h_sig(*pzclassicParams, joinSplitPubKey_);
    ...
    for (int i = 0; i < ZC_NUM_JS_OUTPUTS; i++) {
        ...
        libzclassic::SproutPaymentAddress zaddr = output.addr;  // randomized output
