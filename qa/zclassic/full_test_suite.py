#!/usr/bin/env python2
#
# Execute all of the automated tests related to Zclassic.
#
...
def test_rpath_runpath(filename):
    output = subprocess.check_output(
        [repofile('qa/zclassic/checksec.sh'), '--file', repofile(filename)]
    )
...
def test_fortify_source(filename):
    proc = subprocess.Popen(
        [repofile('qa/zclassic/checksec.sh'), '--fortify-file', repofile(filename)],
...
def check_security_hardening():
    ...
    ret &= test_rpath_runpath('src/zclassic-gtest')
    ...
    ret &= test_fortify_source('src/zclassic-gtest')
...
STAGE_COMMANDS = {
    ...
    'gtest': [repofile('src/zclassic-gtest')],
