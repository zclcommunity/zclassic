...
def main(args=sys.argv[1:]):
    """
    Perform the final Zclassic release process up to the git tag.
    """
...
def build():
    ...
    sh_progress([
        ...
        'config.status: creating libzclassicconsensus.pc',
...
def patch_gitian_linux_yml(release, releaseprev):
    ...
    with PathPatcher(path) as (inf, outf):
        ...
        assert secondline == 'name: "zclassic-{}"\n'.format(
            releaseprev.novtext
        ), repr(secondline)

        outf.write('name: "zclassic-{}"\n'.format(release.novtext))
        outf.write(inf.read())
...
def initialize_logging():
    logname = './zclassic-make-release.log'
    ...
    logging.info('zclassic make-release.py debug log: %r', logname)
